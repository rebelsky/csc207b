...
Your sorts should operate over *generic arrays* and be implemented in a file called `Sorts.java`.
...
* `void eventSort(T[] arr, List<SortEvent<T>> events)`, : given an array list of `T`s and list of `SortEvent<T>` objects, apply those events to the list in-order.
...
In particular, your tests should ensure that if an algorithm generates a list `l` of events, then `eventSort(originalArr, l)` should sort the (original) array as long as the algorithm correctly sorted the array to begin with.
It will be useful to create a copy of your array using the `clone()` method for arrays.

## Part 3: The visualizer and the audibilizer
