...
Rouge::Token::Tokens.token(:ErrorMarker, 'marker')
Rouge::Token::Tokens.token(:OutputMarker, 'marker')

class DrRacket < Rouge::RegexLexer
  title "DrRacket"
  ...
  state :root do
    # When a line starts with Error!, mark the rest of the line as Error
    rule /^Error! /, ErrorMarker, :error
    
    # When a line starts with Output!, mark the rest of the line as Output
    rule /^Output! /, OutputMarker, :output
    
    # Language directive
    rule /^#lang racket/, Other
    ...
    rule /-?\d+\.\d+(e-?[0-9]+)?/, Num::Float
    rule /-?\d+(\/\d+)?/, Num::Integer
